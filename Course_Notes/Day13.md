...
+ <mark>Only</mark> Class A, B, and C addresses can be assigned to a device as an IP Address.

+ Graphic: IPv4 Address Class Prefixes
  ![image](\Day13_Image01.png)
...
+ Graphic: IPv4 Address Class Numbers
...
+ The problem with "classful" assignment is that it led to IP Address wastefulness.
  + E.g., A company requiring 5000 address was assigned a CLASS B IP, leaving 60000+ addresses unused.
+ **Point-to-Point Network** A network that connects 2 points.
  + E.g., A network connecting 2 routers.


---
## CIDR
+ The IETF introduce CIDR in 1993 to replace the "classful" addressing system.
+ Classless Inter-Domain Routing (CIDR) removed the requirements of CLASS A, B, and C addresses regarding their size.
  + This allowed larger networks to be split into smaller networks, allowing greater efficiency.
  + These smaller networks are called "sub-networks" or "subnets"

## Subnetting
+ Graphic: Subnetting Guide
  ![image](\misc01.png)


+ In a /31 subnet there are 0 usable addresses. However, I point-to-point network doesn't need a broadcast or network address, so it's still fine.
+ A /32 mask will never be used on an interface. Instead, it's used to specify a host.
