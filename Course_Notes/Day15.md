# Day 15 | Subnetting Pt. 3 - VLSM

+ While you can use a /31 subnet for a point-to-point connection, on the CCNA test it's best if you use a /30.
...
## VLSM
+ Until now, we have practiced subnetting using Fixed-Length Subnet Masks (FLSMs).
  + This means that all of the subnets use the same prefix length (e.g., subnetting a Class C network into 4 subnets using /26).
+ Variable-Length Subnet Masks (VLSM) is the process of creating subnets of different sizes, to make use of network addresses more efficiently.
